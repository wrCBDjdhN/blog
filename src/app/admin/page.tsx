...
export default async function AdminPage() {
  ...
  const [postCount, productCount, totalViews, recentPosts, recentProducts, friendLinkCount, recentFriendLinks] = await Promise.all([
    prisma.post.count(),
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm">
          <div className="text-sm text-gray-500">友链数量</div>
          <div className="text-3xl font-bold text-gray-900">{friendLinkCount}</div>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">友链管理</h2>
            <Link href="/admin/friendlinks" className="text-sm text-primary-600 hover:text-primary-700">
              管理友链
            </Link>
          </div>
          {recentFriendLinks.length > 0 ? (
            <div className="space-y-3">
              {recentFriendLinks.map((link) => (
                <Link
                  key={link.id}
                  href="/admin/friendlinks"
                  className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0"
                >
                  <span className="text-sm text-gray-900 truncate">{link.name}</span>
                  <span className="text-xs text-gray-500">
                    {link.url}
                  </span>
                </Link>
            ...
            <p className="text-sm text-gray-500">暂无友链</p>
          )}
        </div>
      </div>
    </div>
